...
func TestNew_InvalidSchema(t *testing.T) {
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "failed to compile JSON schema")
}
...
func TestValidate_ValidData(t *testing.T) {
	...
	root := tree.New()
	root.Set(path.NewKeyPath("name"), "Alice")
	root.Set(path.NewKeyPath("age"), 30)
...
func TestValidate_InvalidData(t *testing.T) {
	...
	root := tree.New()
	root.Set(path.NewKeyPath("age"), -5) // Also violates minimum.
...
func TestValidate_ErrorPathMapping(t *testing.T) {
	...
	person := tree.New()
	root.Set(path.NewKeyPath("person"), person)
