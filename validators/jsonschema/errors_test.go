...
func TestValidationErrors_All(t *testing.T) {
	...
	ve := &validationErrors{
		errors: []validator.ValidationError{
			{
				Path:    path.KeyPath{"field"},
				Range:   validator.NewTODORange(),
	...
	assert.Equal(t, path.KeyPath{"field"}, all[0].Path)
}
...
func TestValidationErrors_Error_Multiple(t *testing.T) {
	...
	ve := &validationErrors{
		errors: []validator.ValidationError{
			{
				Path:    path.KeyPath{"field1"},
				Range:   validator.NewTODORange(),
				Code:    errorCodeRequired,
				Message: "missing field1",
			},
			{
				Path:    path.KeyPath{"field2"},
				Range:   validator.NewTODORange(),
				Code:    errorCodeType,
...
func TestAddErrors_SingleError(t *testing.T) {
	...
	assert.Equal(t, path.KeyPath{"field"}, ve.errors[0].Path)
...
func TestAddErrors_MultipleErrors(t *testing.T) {
	...
	for _, err := range ve.errors {
		if err.Code == errorCodeRequired {
			...
			assert.Equal(t, path.KeyPath{"person"}, err.Path)
...
func TestAddErrors_WithBasePath(t *testing.T) {
	...
	require.Len(t, ve.errors, 1)
	assert.Equal(t, path.KeyPath{"parent", "field"}, ve.errors[0].Path)
}
