...
func TestToAny_Nested(t *testing.T) {
	...
	root := tree.New()
	root.Set(path.NewKeyPath("server/port"), 8080)
	root.Set(path.NewKeyPath("server/host"), "localhost")
	root.Set(path.NewKeyPath("database/name"), "testdb")
...
func TestToAny_MissingChild(t *testing.T) {
	...
	node.Set(path.NewKeyPath("existing"), "value")
...
func TestToAny_DeepNesting(t *testing.T) {
	...
	root := tree.New()
	root.Set(path.NewKeyPath("a/b/c/d"), "deep")
	root.Set(path.NewKeyPath("a/b/x"), 123)
	root.Set(path.NewKeyPath("a/y"), true)
