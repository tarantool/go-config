...
func (mc *MutableConfig) Set(path KeyPath, value any) error {
	...
	if mc.validator != nil {
		...
		if len(validationErrs) > 0 {
			// Rollback: remove the set value (simplified).
			// Note: This is simplified; real implementation needs proper rollback.
			node := mc.root.Get(path)
			if node != nil {
				node.Value = nil
				// TODO: properly revert to previous state.
			}
...
func (mc *MutableConfig) Merge(other *Config) error {
	...
	// Perform merge: iterate over all leaf nodes in other config and set them.
...
func (mc *MutableConfig) Update(other *Config) error {
	...
	for val := range ch {
		path := val.Meta().Key
		// Check if path exists in current config.
