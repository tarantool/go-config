...
func TestDirectory_Read_SkipsInvalidYaml(t *testing.T) {
	t.Parallel()

	dir := t.TempDir()
	writeTestFile(t, dir, "invalid.yaml", "bad: yaml: [unclosed")
	writeTestFile(t, dir, "valid.yaml", "mykey: value")
	...
	err := values[0].Get(&dest)
	require.NoError(t, err)
	assert.Equal(t, "value", dest)
}

func TestDirectory_Read_SkipsEmptyFiles(t *testing.T) {
	t.Parallel()
